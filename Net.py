...
class Net():
    '''
    def addNormal(self,x, train_phase=tf.Variable(True)):##x is input-tensor, train_phase is tf.Variable(True/False)    
        beta = tf.Variable(tf.constant(0.0, shape=[x.shape[-1]]), name='bias_beta', trainable=True)    
        gamma = tf.Variable(tf.constant(1.0, shape=[x.shape[-1]]), name='bias_gamma', trainable=True)    
        axises =list(range(len(x.shape) - 1))# np.arange(len(x.shape) - 1)    
        batch_mean, batch_var = tf.nn.moments(x, axises, name='bias_moments')    
